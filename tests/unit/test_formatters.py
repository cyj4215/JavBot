...
class TestFormatPushNotification:
    ...
    def test_scheme_only_url_no_detail_button(self):
        work = MergedWork(id="TEST-009", url="https://")
        _, keyboard = format_push_notification("TestActress", work, _t=self._t)
        assert len(keyboard.inline_keyboard[0]) == 2

    def test_url_with_whitespace_no_detail_button(self):
        work = MergedWork(id="TEST-010", url="http:// ")
        _, keyboard = format_push_notification("TestActress", work, _t=self._t)
        assert len(keyboard.inline_keyboard[0]) == 2
