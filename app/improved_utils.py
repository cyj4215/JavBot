    ...
    for attempt in range(max_retries):
        try:
            ...
            if resp.status_code != 200:
                logger.warning(
                    f"下载失败（第{attempt + 1}次）：状态码 {resp.status_code}, URL: {url}"
                )
            ...
            if len(resp.content) < 1024:
                logger.warning(
                    f"响应内容过小（第{attempt + 1}次）：{len(resp.content)} bytes, URL: {url}"
                )
                if attempt == max_retries - 1:
            ...
            if isinstance(e, requests.Timeout):
                ...
                logger.error(
                    f"未知错误（第{attempt + 1}次）：{url}，错误类型：{type(e).__name__}，错误：{e}"
                )
