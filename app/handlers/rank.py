...
async def _handle_rank_error(target: Message | CallbackQuery, limit: int, page: int, is_edit: bool = False, loading: bool = False) -> None:
    ...
    if is_edit:
        try:
            ...
            logging.getLogger(__name__).debug("编辑排行榜消息失败", exc_info=True)
    else:
...
async def _send_rank_result(target: Message | CallbackQuery, stars: list[dict[str, Any]], limit: int, page: int, with_avatars: bool = False, is_edit: bool = False, msg: Message | None = None) -> None:
    if not stars:
        cached_stars = None
        from . import _get_shared
        shared = _get_shared()
...
async def rank_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE, msg, shared) -> None:
    ...
    try:
        ...
        if not stars and not has_cache:
            ...
            await _send_rank_result(waiting, stars, limit, page)
...
async def rank_page_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if retry_match:
        ...
        try:
            stars = await shared.service.get_hot_star_rankings(limit, page)
            await _send_rank_result(q, stars, limit, page, is_edit=True)
        except Exception as exc:
    ...
    try:
        stars = await shared.service.get_hot_star_rankings(limit, page)
        await _send_rank_result(q, stars, limit, page, with_avatars=with_avatars, is_edit=True, msg=q.message)
    except Exception as exc:
