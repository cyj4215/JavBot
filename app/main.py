...
class ActressService:
    ...
    def get_hot_star_rankings(self, limit: int = 20, page: int = 1) -> List[dict]:
        ...
        try:
            ...
            code, actresses = self.dmm.get_rank(rank_type="actress", limit=fetch_limit)
            if code != 200 or not actresses:
                return []
            result = []
...
def format_profile(profile: ActressProfile, user_id: int = None) -> Tuple[str, Optional[InlineKeyboardMarkup]]:
    ...
    if not profile.found:
        ...
        if profile.suggestions:
            ...
            lines.extend([f"• {html.escape(name)}" for name in profile.suggestions[:8]])
        else:
...
def format_rankings(stars: List[dict], page: int) -> str:
    ...
    lines.append("")
    lines.append(f"<i>抓取时间：{datetime.now().strftime('%Y-%m-%d %H:%M')}</i>")
    return "\n".join(lines)
...
def download_image(url: str, proxy_addr: str = "") -> Optional[bytes]:
    """下载图片到内存，jvav已处理域名替换，直接使用即可"""
    if not url:
        return None
...
def build_app() -> Application:
    ...
    async def run_search_reply(msg, query: str, user_id: int = None) -> None:
        ...
        try:
            ...
            if send_latest_covers and profile.found and profile.latest_works:
                # 有封面的情况：把简介放在第一张封面的caption里，完全融合
                first_work = None
                other_works = []
                for work in profile.latest_works:
                    img = (work.get("img") or "").strip()
                    if img and first_work is None:
                        first_work = work
                    else:
                        other_works.append(work)
                    if len(other_works) >= max(1, latest_cover_limit) - 1:
                        break

                if first_work:
                    # 第一张图的caption包含完整简介 + 第一张作品信息
                    img = (first_work.get("img") or "").strip()
                    av_id = (first_work.get("id") or "").strip()
                    av_date = (first_work.get("date") or "未知").strip()
                    av_title = (first_work.get("title") or "").strip()[:60]

                    caption_lines = [base_text, "", "<b>🎬 最新作品</b>"]
                    caption_lines.append(f"<b>🎬 {html.escape(av_id)}</b>")
                    if av_date:
                        caption_lines.append(f"📅 出品时间：{html.escape(av_date)}")
                    if av_title:
                        caption_lines.append(f"📝 {html.escape(av_title)}")
                    caption = "\n".join(caption_lines)

                    try:
                        img_bytes = await asyncio.to_thread(download_image, img, proxy_addr)
                        if img_bytes:
                            await waiting.edit_media(
                    ...
                    for work in other_works:
                        ...
                        av_title = (work.get("title") or "").strip()[:60]
                        if not img:
                            continue
                        caption_lines = [f"<b>🎬 {html.escape(av_id)}</b>"]
                        if av_date:
                            caption_lines.append(f"📅 出品时间：{html.escape(av_date)}")
                        if av_title:
                            caption_lines.append(f"📝 {html.escape(av_title)}")
    ...
    async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        text = (
            ...
            "命令：/search 关键词（磁力）\n"
            "命令：/rank [数量] [页码]（热门女优榜）\n"
            "收藏功能：/fav 名字（收藏） /unfav 名字（取消） /myfav（查看） /favlatest（最新作品）"
        )
    ...
    async def rank_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            await waiting.edit_text("获取排行榜失败，请稍后再试。")
    ...
    async def favorite_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        # 先查询女优信息
        waiting = await msg.reply_text(f"正在查询 {query}...")
        try:
            profile = await asyncio.to_thread(service.query_profile, query)
            if not profile.found:
                await waiting.edit_text(f"未找到女优: {query}")
                return
            ...
            if success:
                await waiting.edit_text(
                    f"✅ 已收藏: {profile.star_name}\n\n"
                    f"使用 /myfav 查看所有收藏\n"
                    f"使用 /favlatest 查看收藏女优的最新作品"
                )
            else:
                await waiting.edit_text("收藏失败，请稍后再试。")
    ...
    async def unfavorite_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        # 先检查是否已收藏
        if not favorites_manager.is_favorite(user.id, query):
            # 尝试匹配收藏中的名字
            favorites = favorites_manager.get_favorites(user.id)
            matched = None
            for fav in favorites:
                if query.lower() in fav['actress_name'].lower() or fav['actress_name'].lower() in query.lower():
            ...
            if matched:
                ...
                await msg.reply_text(f"未找到收藏: {query}")
                return

        # 移除收藏
        success = favorites_manager.remove_favorite(user.id, query)

        if success:
            await msg.reply_text(f"✅ 已取消收藏: {query}")
        else:
            await msg.reply_text("取消收藏失败，请稍后再试。")

    async def my_favorites_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """查看我的收藏命令"""
        msg = update.effective_message
        if not msg:
            return
        ...
        favorites = favorites_manager.get_favorites(user.id, limit=20)
        ...
        lines.append("")
        lines.append(f"共收藏 {len(favorites)} 位女优")
        lines.append("")
    ...
    async def rank_page_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            try:
                await q.answer("加载失败，请稍后重试", show_alert=True)
            except Exception:
                pass
    ...
    async def post_init(application: Application) -> None:
        ...
        try:
            ...
            logging.warning("failed to setup command menu: %s", exc)
    ...
    app.add_handler(CallbackQueryHandler(favorite_query_callback, pattern=r"^fav"))
